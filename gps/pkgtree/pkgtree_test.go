...
import (
	...
	_ "github.com/nomango/dep/internal/test" // DO NOT REMOVE, allows go test ./... -update to work
)
...
func TestToReachMap(t *testing.T) {
	...
	allex := map[string][]string{
		b(""):               {"encoding/binary", "github.com/Masterminds/semver", "github.com/nomango/dep/gps", "go/parser", "hash", "net/http", "os", "sort"},
		b("m1p"):            {"github.com/nomango/dep/gps", "os", "sort"},
		b("namemismatch"):   {"github.com/Masterminds/semver", "os"},
		b("otherpath"):      {"github.com/nomango/dep/gps", "os", "sort"},
		b("simple"):         {"encoding/binary", "github.com/nomango/dep/gps", "go/parser", "hash", "os", "sort"},
		b("simple/another"): {"encoding/binary", "github.com/nomango/dep/gps", "hash", "os", "sort"},
	}
...
func TestFlattenReachMap(t *testing.T) {
	...
	for _, testCase := range []*flattenReachMapCase{
		...
		// ignore two that should knock out gps
		{
			...
			expect:     except("sort", "github.com/nomango/dep/gps", "go/parser"),
			isStdLibFn: nil,
			main:       true,
			tests:      true,
			ignore: NewIgnoredRuleset([]string{
	...
	t.Run("disallowed", testFlattenReachMap(&ptree, &flattenReachMapCase{
		name:       "disallowed",
		expect:     []string{"github.com/nomango/dep/gps", "hash", "sort"},
		isStdLibFn: nil,
		main:       false,
		tests:      false,
	}))
