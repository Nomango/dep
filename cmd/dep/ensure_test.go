...
func TestValidateUpdateArgs(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			args:      []string{"github.com/nomango/dep/cmd"},
			wantError: errUpdateArgsValidation,
			wantWarn: []string{
				"github.com/nomango/dep/cmd is not a project root, try github.com/golang/dep instead",
			},
		},
		{
			name:      "not present in lock",
			args:      []string{"github.com/nomango/dep"},
			wantError: errUpdateArgsValidation,
			wantWarn: []string{
				"github.com/nomango/dep is not present in Gopkg.lock, cannot -update it",
			},
		},
		{
			name:      "cannot specify alternate sources",
			args:      []string{"github.com/nomango/dep:github.com/example/dep"},
			wantError: errUpdateArgsValidation,
			wantWarn: []string{
				"cannot specify alternate sources on -update (github.com/example/dep)",
			},
			lockedProjects: []string{"github.com/nomango/dep"},
		},
		{
			name:      "version constraint passed",
			args:      []string{"github.com/nomango/dep@master"},
			wantError: errUpdateArgsValidation,
			wantWarn: []string{
		...
		},
		{
			...
			args:      []string{"github.com/nomango/dep@master", "-v"},
			wantError: errUpdateArgsValidation,
			wantWarn: []string{
