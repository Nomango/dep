...
func TestReadManifest(t *testing.T) {
	...
	want := Manifest{
		Constraints: map[gps.ProjectRoot]gps.ProjectProperties{
			gps.ProjectRoot("github.com/nomango/dep"): {
				Constraint: c,
			},
...
func TestWriteManifest(t *testing.T) {
	...
	c, _ := gps.NewSemverConstraint("^0.12.0")
	m := NewManifest()
	m.Constraints[gps.ProjectRoot("github.com/nomango/dep")] = gps.ProjectProperties{
		Constraint: c,
	}
	m.Constraints[gps.ProjectRoot("github.com/babble/brook")] = gps.ProjectProperties{
		Constraint: gps.Revision("d05d5aca9f895d19e9265839bffeadd74a2d2ecb"),
	}
	m.Ovr[gps.ProjectRoot("github.com/nomango/dep")] = gps.ProjectProperties{
		Source:     "https://github.com/golang/dep",
		Constraint: gps.NewBranch("master"),
	}
...
func TestCheckRedundantPruneOptions(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			wantWarn: []error{
				fmt.Errorf("redundant prune option %q set for %q", "unused-packages", "github.com/nomango/dep"),
				fmt.Errorf("redundant prune option %q set for %q", "non-go", "github.com/nomango/dep"),
				fmt.Errorf("redundant prune option %q set for %q", "go-tests", "github.com/nomango/dep"),
			},
		},
		{
...
func TestValidateProjectRoots(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			manifest: Manifest{
				Constraints: map[gps.ProjectRoot]gps.ProjectProperties{
					gps.ProjectRoot("github.com/nomango/dep/foo"): {
						Constraint: gps.Any(),
					},
			...
			wantWarn: []string{
				"the name for \"github.com/nomango/dep/foo\" should be changed to \"github.com/nomango/dep\"",
...
//DefaultOptions: 15,
//PerProjectOptions: map[gps.ProjectRoot]gps.PruneOptionSet{
//"github.com/nomango/dep": gps.PruneOptionSet{
//NestedVendor:   pvtrue,
//UnusedPackages: pvfalse,
//NonGoFiles:     pvfalse,
//GoTests:        pvfalse,
